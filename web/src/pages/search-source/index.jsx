...
import {
  ...
  Tag, Modal, Spin, Alert, Tooltip, Typography, message,
} from 'antd'
...
const MetaSourceTab = ({ refreshKey }) => {
  ...
  const handleToggle = async (key, enabled) => {
    ...
    try {
      await systemApi.saveSource({
        name: key,
        enabled,
        override: { base_url: record.base_url, api_key: record.api_key },
      })
    } catch {
      message.error('保存失败')
    }
  ...
  const handleEditOk = async () => {
    ...
    try {
      await systemApi.saveSource({
        name: editingRecord.key,
        enabled: editingRecord.enabled,
        override: { base_url: values.base_url || '', api_key: values.api_key || '' },
      })
      message.success('已保存')
    } catch {
      message.error('保存失败')
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v, record) => (
        <Switch size="small" checked={v} onChange={(checked) => handleToggle(record.key, checked)} />
      ),
    },
    {
